...
| `/apply-captures` | Processes extraction reports with status pending-review — Resolution R15 (accept/reject/defer) by a LIVE PILOT, records… |
| `/archgate` | Assesses an architectural decision across 7 characteristics (v3 — Profile without aggregate score, conjunctive s… |
...
| `/author-mode` | Instructions for an IWE Template author: staging channel (trial → FMT), author rules L3, Extensions Gate. Load… |
...
| `/peer-conversation` | Multi-turn dialogue of a writer (Claude) with one or more Peer partners (any combination of kimi/codex/hermes/c… |
...
| `/vdv` | OWC Skill — generator and auditor for staged process descriptions based on the 6 principles of Input·Action·Output. Use for… |
...
| `/verify-hypotheses` | Reconciles the hypotheses log (hypotheses-log.md) on demand outside the week-close rhythm. Filters entries with status "pending… |
...
| `/wp-new` | Creates a new Work Product (WP) with atomic writes to 4 local locations (+ conditional post-step to external… |
