...
> **IWE (Intellectual Work Environment)** — a personal environment for intellectual work, analogous to an IDE for developing thinking. Just as an IDE gives a programmer an editor, compiler, linter, and debugger — IWE gives a person formalized knowledge (Pack), automatic extraction (Extractor), correctness verification (FPF/SPF), and gap diagnosis (Digital Twin). The person works together with AI agents, each of which plays its own Role.
>
...
> **Terminology:** IWE = Intellectual Work Environment, described through 5 architectural viewpoints: systems, descriptions, Roles, Methods, Work Products (§ 1.2). Triad A.7: Role → Method → Work Product. Exocortex = the description storage system inside IWE (CLAUDE.md + memory/). For more detail: [DP.IWE.001](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.IWE.001-intelligent-working-environment.md).
...
>
> Links with `./` — files in this repository. Links with `github.com/...` — other Repositories.
...
IWE augments the user's thinking rather than replacing it. The Distinction:

- **Prosthesis:** AI thinks for you → task is done, but you did not learn → atrophy
...
3. **Fading scaffolding.** Training: more support at early levels, less at advanced ones. Tiers T0→T4: from direct answers to co-thinking.

**Criterion:** after interacting with IWE, the user has become more competent — not merely obtained a result.
...
> **Three IWE classifications:** Viewpoints (this section) answer "through which lens are we looking." Perimeters L1–L4 (§ 2.1) — "where it lives." Tiers T0–T4 + TM/TA/TD (§ 9.1) — "what access level."
...
A Role describes a function (WHAT to do), not the performer (WHO). One performer (holder) can play multiple Roles. One Role can be played by different performers (Claude, a bash script, a person). For more detail: [DP.ROLE.001 §3](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.ROLE.001-platform-roles.md).
...
| **Guide** | R13 | Telegram bot | Navigates users through Platform Services | On user request |
| **User** | — | Human | Decision-making, creation, reflection | Always |

> **Test:** Is it a function described without naming the performer? → Role.
>
> **Role ≠ Performer (HD #5).** The notation "Strategist (R1) ← Claude" reads: the Role is Strategist, the holder is Claude. "Human" is not a Role — it is a performer playing the "User" Role.
>
...
| **ArchGate** (EMOSSS) | Evaluation of architectural decisions across 7 characteristics | R1 Strategist | Evaluation table, decision |
...
>
> **Why a separate viewpoint?** Triad A.7 (Role → Method → Work) is the central Distinction of FPF. Without the "Methods" viewpoint, Protocols get lost among Descriptions — yet they are not merely knowledge; they are **procedures** that connect Roles to Work Products.
...
|-------------|----------|----------------|---------|
| **Strategy hub** | `DS-strategy/` | R1 Strategist + User | Storage of personal documents (plans, strategy, inbox) and running strategy Sessions |
| **Pack documents** | `PACK-{domain}/` | R2 Extractor + User | Accumulation of formalized Domain descriptions (single source of truth) |
...
| **Notes** | `DS-strategy/inbox/` | Bot (from Telegram) | Quick capture of thoughts and observations for later Strategist processing |
...
> **Integrity principle:** Remove any viewpoint and IWE degrades. Without systems — no execution. Without descriptions — stateless assistant. Without Roles — task chaos. Without Methods — ad hoc work. Without Work Products — no result.
...
| **USER-SPACE** | `CLAUDE.md` § "My Rules" (section `<!-- USER-SPACE -->`) | **Does not touch** | Your rules, Distinctions |
| **CONFIG** | `memory/day-rhythm-config.yaml` | Does not touch | Configure parameters |
| **PERSONAL** | `memory/MEMORY.md`, AUTHOR-ONLY zones in Protocols | Does not touch | You edit |
...
> **AUTHOR-ONLY zones** — blocks inside PLATFORM files marked with `<!-- AUTHOR-ONLY -->` markers. They are preserved by update.sh. Details: [CLAUDE.md §7](../CLAUDE.md).
...
The Script downloads an update Manifest from GitHub, compares sha256 hashes of local files against upstream, shows a preview, and applies changes after confirmation:

| Step | What it does | Result |
|------|-------------|--------|
| 0. Self-update | Checks for a newer version of update.sh | Script is always current |
...
| 3. Preview | Shows: new files, updated, untouched | You decide: apply or not |
...
**Your rules:** add them to the "8. My Rules" section at the end of CLAUDE.md (after the `<!-- USER-SPACE -->` marker). This section is preserved during updates. Rules in `<repo>/CLAUDE.md` of specific repos are not affected at all.
...
Changes in the Template accumulate. You can update once a day, once a week, or once a month — a single `bash update.sh` command will apply everything accumulated during that period. CHANGELOG.md will show what changed.

**Telegram notifications:**

Every morning at 7:28, bot @aist_me_bot sends a digest of changes from the last 24 hours (if any occurred). Subscribe to the updates channel to stay informed. A notification is information. The decision to update is always yours.
...
**Marp — preparing presentations:** Marp converts Markdown files to slides (PDF, HTML, PPTX). Workflow: write `.md` with `---` separator → preview in VS Code (Marp extension) → export with `marp --pdf slides.md`. Sliduments (MIM.WP.001) are text-based, so Markdown + Git = versions, diffs, edits through Claude Code. Setup: `npm install -g @marp-team/marp-cli` + VS Code → Extensions → "Marp for VS Code".
...
All knowledge is organized into 4 levels. Each level is Constrained by the one above:
...
FPF (First Principles Framework) — the "operating system for thinking." It defines basic constructs and rules for combining them.
...
**How to read:** NOT sequentially. Start with the table of contents, then find the needed sections by searching for codes (for example, `FPF A.7` = Strict Distinction).
...
**Key point:** **Base = the Platform provides** (principles, frameworks, Templates). **Pack and DS = the user creates.** Pack is the **only** source of truth for Domain knowledge. DS consumes, it does not create.
...
> **Your own formats:** A user can create their own format — this would be a DS repo with `template: true` in REPO-TYPE.md.
...
|-----------|---------------|-----|
| You have defined a knowledge domain | `PACK-{domain}` | `/pack-new` — guided flow through SPF (verifies/clones SPF+FPF, defines the Domain, creates scaffold) |
...
|---|----------------|-------------|
| **Location** | In each system repo | `DS-strategy/docs/` |
| **Author** | System owner | Strategist (aggregation) |
| **Content** | "Where THIS system is headed" | "Where I am headed" |
...
### 5.1. ORW Fractal: Day and Session (Two of Four Scales)

ORW (Opening → Work → Closing) is a **fractal pattern** that operates at four scales: Session, Day, Week, Month. A Day consists of Sessions; each Session is a complete ORW cycle within the daily cycle. Weeks and months are closed by the Week Close and Month Close Protocols.
...
| **Session** | Opening | Any assignment (no exceptions) | R6 Coder |
| **Session** | Work | After completing Opening | R6 Coder |
| **Session** | Closing | "closing" / "done" / "close" | R6 Coder |

> **Distinction: Day ≠ Session.** Day Open/Close are separate ritual Sessions (trigger only, no assignment). Session Open/Close are always in the context of specific work.
...
4. **Strategy session** — if today is `strategy_day` (from `day-rhythm-config.yaml`) → **do NOT create DayPlan** (the daily plan is already in WeekPlan → section "Plan for Monday"). Show WeekPlan, skip step 7
...
2. **What was learned** — captures in Pack, Distinctions, insights, guides
...
| # | Rule | Essence |
|---|------|---------|
| 1 | Slot 1 = self-development | Do not move to routine tasks until the slot is done |
...
2. Match found → proceed + **DayPlan Gate:** if the Work Product is not in the current day's DayPlan → add a row
...
Before working, Claude announces:
...
4. Own IWE user → plans, Review, posts, captures (WITHIN the system)
...
> - **Entire Session on another model:** If at opening Claude determines the task is trivial/closed-loop and the current model is excessive, it says: *"This task is trivial — I recommend switching to Haiku via `/model`. I cannot switch automatically."* The user switches manually → the entire Session runs on the cheaper model.
> - **Individual task within a Session:** A trivial task appears mid-Session → Claude delegates to a sub-agent on a cheaper model. The Session is not interrupted. Delegation is downward only: Opus→Sonnet/Haiku, Sonnet→Haiku. Switching upward — only via `/model`.
...
**Session registration:** after alignment → a row in `<governance-repo>/inbox/open-sessions.log`.
...
|-------|------|---------|-------|----------|
| 1 | `MEMORY.md` | Week tasks, lessons, navigation | ≤100 rows | Every Session (auto) |
| 2 | `CLAUDE.md` | Slim core: Blocking rules + navigation | ~90 rows | At startup (auto) |
| 3 | `memory/*.md` | Protocols, Distinctions, SOTA, Roles, checklists | By token budget (HOT/WARM/COLD) | On triggers from CLAUDE.md |
...
| `checklists.md` | Quality checklists | Before answering, before modifying |
...
> **`roles.md` — an evolving file.** The Template ships Platform Roles (R1–R21). Add your own Roles in the "User Roles" section (R100+). This helps Claude select the right behavior for each task — not guess, but cross-reference the table.

**Policy:** References ≤100 rows, Protocols ≤150, registries ≤200 + cleanup on Close; total budget by tokens (HOT/WARM/COLD, `memory/memory-lifecycle-spec.md`), not by file count. Cross-system → memory/. System-specific → `<repo>/CLAUDE.md`.
...
> **Dual KE routing (HD #29):** Pack knowledge ≠ Implementation decision. The Extractor (R2) at Close proposes recording knowledge in two places: Domain → Pack, implementation → DS docs/. One Pipeline, two outputs.
...
- Linked Packs (knowledge sources)
...
| **Day Close** | Evening (trigger "closing the day") | Results → what was learned → praise → setup for tomorrow | Updated DayPlan |
...
| **Note-Review** | As needed | Process fleeting notes and captures | Routed to Pack/inbox |
| **Add-WP** | When a new task appears | Add Work Product to plan (4 places) | Updated WeekPlan + WP file |
...
**Single-source pattern:** DS-strategy (hub) is the sole registry (`WP-REGISTRY.md` + `inbox/WP-*.md`), aggregated via `scripts/active-wp-sweep.sh`. The hub-and-spoke model with WORKPLAN.md was retired by WP-283 F-H (May 2026).
...
- **Day Open does not create a DayPlan** — the daily plan is already embedded in WeekPlan (section "Plan for [day]")
...
Every Work Product in ⏳ pending status has an **activation condition** — an answer to "under what condition does this Work Product enter the WeekPlan?"
...
Conditions are stored in the Work Product context file (`inbox/WP-NNN/WP-NNN.md`, `activation:` field in frontmatter — for example `activation: on-demand` or `activation: dep:WP-73`). WP-REGISTRY is only an index (number/Priority/name/status/repo/budget); details of a specific Work Product, including the activation condition, live in its context file (issue #263).
...
Note-Review classifies each note into one direction. A Draft is only a recommendation — created after confirmation.

| # | Category | Criterion | Destination |
|---|----------|----------|------------|
| 1 | **Dissatisfaction** | Discontent, discomfort, "I want to change this" | `Dissatisfactions.md` |
...
1. **Draft ≠ Stub.** A Draft is a private text (`DS-strategy/drafts/`), can be raw. A Stub is a public text (`DS-Knowledge-Index/`, status: draft), must be coherent. *Test:* can you show it to someone else? No → Draft.
...
Every artifact MUST exit into one of the directions within its TTL. If drafts accumulate, guards trigger:

| Threshold | Response | What to do |
|-----------|---------|-----------|
| ≤5 drafts | Normal | Work by Priority |
| 6–10 drafts | **Warning** | Prioritize or close extras until ≤5 |
| >10 drafts | **Blocked** | Cannot add new ones. First advance or close until ≤5 |

Guards are checked at every Note-Review and when creating a new Draft.

#### Closed Loop: Pack ↔ Content

The Pipeline does not run linearly — it is a **closed loop**:
...
If ≥2 answers are "no" → Pipeline is stalled → add to the strategy Session Agenda.
...
Pack is a formalized Domain passport. The **only source of truth** for Domain knowledge.
...
- Roles (who acts)
- Methods (how to act)
- Work Products (outputs of Methods)
...
**Quick start:** `/pack-new` — the Skill will guide you through domain selection, Pack name, scaffold creation, and display a Roadmap for phases F1–F6.
...
| Bulk-Extraction | Document processing | After analysis — an Extraction Report |
...
Claude Code connects to the Platform Gateway MCP server (via https://claude.ai/settings/connectors). The Gateway `iwe-knowledge` (`mcp.aisystant.com/mcp`) aggregates all backends — one connection point for all knowledge tools.
...
| `knowledge_get_document` | Retrieve a specific document by name | `knowledge_get_document("DP.ROLE.001-platform-roles.md")` |
...
| **Templater (R9)** | Template update | Drift detection, validation | During `update.sh` |
...
|---------------|---------|
| **E**volvability | What breaks when things change? |
| **M**onitorability | What happens at 10x scale? |
| **O**nboardability | How long to read before starting? Exoskeleton or prosthesis? |
| **S**calability | Does it create a Platform for new things? |
| **S**peed | What is the latency? (bot <3 sec, CLI <1 sec) |
| **S**tateness | How do the best solve this? Check SOTA. |
| **S**ecurity | What are the threats? PII, secrets, injection surface? |

**Format:** Decision → principles (step 1) → evaluation table (step 2) → weak points → how to strengthen them (step 3).

**Coordination cost check** (for multi-agent decisions): are coordination costs < gains from parallelism? Three conditions: (1) context isolation, (2) parallelism gain, (3) tool specialization. If all three are NOT met → single-agent.

### 8.2. SOTA Practices

Priority triple (check ALWAYS for architectural decisions):

| # | Practice | Essence |
|---|----------|---------|
| 1 | Context Engineering | Write/Select/Compress/Isolate — what enters an agent's context |
...
| Current conversation context | History of past conversations (reset in a new Session) |
| Contents of repos you have granted access to | Repos outside the working directory |

> **Anthropic API:** Anthropic [does not use data from the API](https://www.anthropic.com/policies/privacy-policy) to train models. Data is processed but not retained for training.
...
4. Creates the scaffold `PACK-{slug}/` + starter files
5. Shows the Roadmap for phases F1–F6
...
| F2. Entities | Roles, Work Products, Methods — a list (SPF §04) | 1–2h |
...
Tool for filling in: `/ke` — records knowledge into Pack during work.

### 10.2. New Agents and Tools

Before adding anything — IntegrationGate (§ 8.4). After defining:
...
**Principle:** Minimum complexity at start. The Strategist alone is sufficient for the first months. The Extractor — when Pack reaches 10+ entities. The Synchronizer — when there are 3+ Repositories.
...
| Question | Answer | Where |
|---------|-------|-------|
| Which SOTA applies? | Priority triple | [sota-reference.md](../memory/sota-reference.md) |
...
| What are FPF, SPF, ZP? | Three levels of principles: ZP → FPF → SPF → Pack. Each level generates the next | § 3.1 |
...
**What happens.** You describe a task in detail in the chat — Claude understands and works on it. A new Session starts — as if nothing happened.
...
**What to do.**
1. **WP file = permanent task memory.** When a Work Product is created through WP Gate, Claude writes the context to `DS-strategy/inbox/WP-{N}-slug.md`. In the next Session it reads that file and restores context.
2. **If a WP file was not created** — the task was probably assessed as ≤2h and ≤1 Session. Say: *"Create a context file for this task."* Or add a rule to `<repo>/CLAUDE.md`: *"Always create a WP file when adding a Work Product."*
...
**What happens.** Obsidian tries to index all of `~/IWE`, including large technical Markdown files. For example, `FPF/FPF-Spec.md` can be many megabytes, and the interface hangs on a white screen.

**What to do.** Open only a specific governance Repository (`DS-strategy`) as a vault in Obsidian. The `~/IWE` root as a vault is not supported. Use VS Code to browse the full workspace; do not add large technical Repositories to the Obsidian vault via symlink.
...
| **3. Gateway MCP** | Pack >100 files | Configure knowledge search via Gateway for your Packs (DP.IWE.002 § 7.1). Claude can search across the full base |
...
|------|-------|----------|
| List of week tasks (Work Products) | `MEMORY.md` | Claude sees it every Session, checks via WP Gate |
...
| Reference material (terms, checklists) | `memory/*.md` | Claude reads it on trigger (§ 5.2) |
| Details of each task | `DS-strategy/inbox/WP-*.md` | Claude reads it when opening a task (Ritual, step 3) |
| Domain knowledge | Pack Repositories | Claude reads it on explicit request or via MCP |
| Strategy, dissatisfactions | `DS-strategy/docs/` | Strategist (R1) uses it during planning |
...
| **Root CLAUDE.md** (standard) | With a caveat | update.sh will overwrite the standard part. Add your supplements to the end of sections |
...
├── current/     ← Current (weekly/daily plan) — changes daily
...
Pattern **Inbox → Processing → Archive**: incoming is processed → result goes to docs/ or Pack → source is archived. Nothing accumulates without control.

You can change the structure, but preserve the separation of "active / incoming / closed" — without it, inbox will grow indefinitely.
...
| 2 | **Day Close** | Evening (trigger) | Results → what was learned → praise → setup for tomorrow | Updated DayPlan |
...
**Answer (revised 23.07 — previous version overstated requirements): Git Bash is sufficient for installation and daily work; WSL is not required.** IWE is bash scripts + Node.js. The MCP server that actually appears in the template's `.mcp.json` (`iwe-knowledge`, HTTP at `mcp.aisystant.com`) is a remote Service — it does not care which terminal launched Claude Code on the client side. The Template contains no local/stdio MCP server for which the terminal would matter. The only real bash dependency is Claude Code hooks (pre/post-commit etc.) that call `.sh` files through the system shell: these work as long as `bash` (the one installed with Git for Windows) is on the system `PATH`. Details → [SETUP-GUIDE.md § Windows](SETUP-GUIDE.md#00-windows-without-wsl).

**When to use WSL:** you need automation without a constantly open window (cron-like local scheduling — not natively available on bare Windows; WSL with `systemd` provides a full `cron`/`launchd` equivalent) **or** you prefer a full Linux Environment for other reasons. For scheduling without local automation there is a simpler option — the cloud approach via GitHub Actions (OS-independent).
...
**Why files in WSL and not on the Windows disk (if you chose WSL)?** The WSL file system (`~/`) is 5–10x faster than access to `/mnt/c/` (Windows disk through WSL). Watch scripts, git operations, and MCP indexing on `/mnt/c/` are critically slow.

**Honest caveat.** Neither path has been tested live on Windows by this team (the Template CI matrix runs only `ubuntu-latest`/`macos-latest`; there is no Windows runner). If you encounter a specific breakage in Git Bash specifically (not a vague "something is wrong", but a reproducible symptom) — file an issue in FMT-exocortex-template; that is more valuable than guessing in advance.

#### "I do not understand what to put in notes"

**What happens.** You see the notes feature in the bot (`.text`), but do not know what to write — everyday tasks? Ideas? Everything?
...
**Do not record:**
- Everyday tasks ("buy oil") — use to-do apps for those (Todoist, Apple Reminders)
...
|-------|-----------------|-----------|
| **Question outside the knowledge base** | Bot answers in general phrases without referencing specific documents | The bot knows what is in the knowledge base (Gateway iwe-knowledge). Make the question more specific: "What does course X say about Y?" instead of the abstract "tell me about Y" |
| **Long answer gets cut off** | Text breaks off mid-sentence | Telegram limits message length. Ask: "continue" or "give a brief version" |
...
**If the answer is completely off** — give a 👎. This triggers automatic classification (feedback_triage), and the problem will appear in the developer's report.
...
3. 3 types of Repositories (§ 4.1) — 15 min
