...
| `check-script-collisions.sh` | `scripts/check-script-collisions.sh` | Check for script collisions between the author zone and the FMT-exocortex-template. |
...
| `day-open-smoke.sh` | `scripts/day-open-smoke.sh` | Core smoke test for Day Open (≤10 s) |
...
| `generate-rules-registry.py` | `.claude/scripts/generate-rules-registry.py` | generate-rules-registry.py — collects rules-registry.yaml from PACK-agent-rules/rules/AR.NNN.md. |
| `generate-skills-catalog.sh` | `scripts/generate-skills-catalog.sh` | skills-catalog.yaml generator |
| `git-dirty-guard.sh` | `scripts/git-dirty-guard.sh` | Protects a repository's periodic pull from a dirty working tree. |
...
| `pack-ci-install.sh` | `scripts/pack-ci-install.sh` | Installs a CI guard (ID collision detector) in all Pack repositories under ~/IWE/ |
| `peer-session-finalize.sh` | `scripts/peer-session-finalize.sh` | Finalize / interrupt for peer sessions (DP.SC.154) |
| `pending-phases-sweep.sh` | `scripts/pending-phases-sweep.sh` | Iterates over active WP-context files and prints pending phases |
...
| `settings-promote.sh` | `scripts/settings-promote.sh` | Register a Hook in the .claude/settings.json template |
...
| `skills-pull.sh` | `scripts/skills-pull.sh` | Sync L1 skills from FMT-exocortex-template into a personal IWE |
| `smoke-clean-env.sh` | `scripts/smoke-clean-env.sh` | Smoke test for new scripts/*.sh in FMT-exocortex-template. See WP-347 PD-1. |
